...
const Flag = ({ code, className = "", style = {}, circular = false }) => {
    ...
    const sizeStyle = circular ? { width: '40px', height: '40px', aspectRatio: '1/1' } : { width: '1.5em' };
    const mediaClass = circular ? "w-full h-full object-cover" : "w-full h-auto";
    ...
    if (normalizedCode === 'GB-ENG') {
        return (
            <div className={`inline-flex items-center justify-center overflow-hidden ${roundedClass} ${className}`} style={{ ...sizeStyle, border: '1px solid #eee', ...style }}>
                <svg viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg" className={mediaClass}>
                    <rect width="256" height="256" fill="#fff" />
                    <rect x="112" width="32" height="256" fill="#ce1124" />
                    <rect y="112" width="256" height="32" fill="#ce1124" />
                </svg>
            </div>
        );
    }
    ...
    if (normalizedCode === 'GB-SCT') {
        return (
            <div className={`inline-flex items-center justify-center overflow-hidden ${roundedClass} ${className}`} style={{ ...sizeStyle, ...style }}>
                <svg viewBox="0 0 5 3" xmlns="http://www.w3.org/2000/svg" className={mediaClass}>
    ...
    return (
        <div className={`inline-flex items-center justify-center overflow-hidden ${roundedClass} ${className}`} style={{ ...sizeStyle, ...style }}>
            <FlagComponent className={mediaClass} />
        </div>
    );
};
